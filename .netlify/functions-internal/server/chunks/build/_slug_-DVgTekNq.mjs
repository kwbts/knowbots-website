...
import { ssrRenderAttrs, ssrInterpolate, ssrRenderClass, ssrRenderAttr, ssrRenderList, ssrRenderVNode, ssrRenderComponent } from 'vue/server-renderer';
...
const _sfc_main$4 = {
  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "media-block my-8" }, _attrs))}>`);
      if (__props.value.image) {
        _push(`<div class="${ssrRenderClass(["image-container", __props.getImageDisplayClass(__props.value.image)])}"><img${ssrRenderAttr("src", __props.getImageParameters(__props.value.image).url)}${ssrRenderAttr("alt", __props.value.image.alt || "Blog content image")} class="image-responsive">`);
        if (__props.value.caption) {
          _push(`<div class="caption-container"><p class="caption-text">${ssrInterpolate(__props.value.caption)}</p></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
      } else if (__props.value.video) {
        _push(`<div class="video-container"><iframe${ssrRenderAttr("src", __props.value.video.url)} frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen class="w-full aspect-video rounded-lg"></iframe>`);
        if (__props.value.caption) {
          _push(`<div class="caption-container"><p class="caption-text">${ssrInterpolate(__props.value.caption)}</p></div>`);
        } else {
          _push(`<!---->`);
  ...
  async setup(__props) {
    ...
    const {
      ...
      ),
      {
        fresh: true,
        server: true
        // Only fetch on server, no client-side refreshing
      }
    ...
    const getImageDisplayClass = (image) => {
      ...
      const dimensions = (_b = (_a = image.asset) == null ? void 0 : _a.metadata) == null ? void 0 : _b.dimensions;
      if (dimensions) {
    ...
    useSeoMeta({
      ...
      articlePublishedTime: () => {
        var _a;
        return (_a = blogPost.value) == null ? void 0 : _a.publishedDate;
      },
      articleModifiedTime: () => {
        var _a;
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<main${ssrRenderAttrs(mergeProps({ class: "flex-grow" }, _attrs))} data-v-7960cfeb><section class="bg-offWhiteBackground py-16 px-8 pt-24" data-v-7960cfeb><div class="max-w-screen-lg mx-auto px-20 blog-content" data-v-7960cfeb>`);
      if (unref(isLoading)) {
        _push(`<div class="text-center" data-v-7960cfeb><p class="text-lg text-gray-500" data-v-7960cfeb>Loading...</p></div>`);
      } else if (unref(error)) {
        _push(`<div class="text-center" data-v-7960cfeb><p class="text-lg text-red-500" data-v-7960cfeb>${ssrInterpolate(unref(error).message)}</p></div>`);
      } else if (unref(blogPost)) {
        _push(`<div data-v-7960cfeb>`);
        if (unref(blogPost).mainImage) {
          _push(`<div class="mb-4" data-v-7960cfeb><div class="${ssrRenderClass(getImageDisplayClass(unref(blogPost).mainImage))}" data-v-7960cfeb><img${ssrRenderAttr("src", getImageParameters(unref(blogPost).mainImage, true).url)}${ssrRenderAttr("alt", unref(blogPost).mainImage.alt || "Blog Image")} class="image-responsive" data-v-7960cfeb></div></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<div class="flex flex-col sm:flex-row sm:space-x-16" data-v-7960cfeb><div class="mb-4 sm:mb-0 tight-spacing" data-v-7960cfeb><p class="text-sm text-darkGray font-body font-bold mb-1" data-v-7960cfeb>Written by</p><p class="text-sm text-darkGray font-body" data-v-7960cfeb>${ssrInterpolate(((_a = unref(blogPost).author) == null ? void 0 : _a.name) || "Unknown Author")}</p></div><div class="tight-spacing" data-v-7960cfeb><p class="text-sm text-darkGray font-body font-bold mb-1" data-v-7960cfeb>Published on</p><p class="text-sm text-darkGray font-body" data-v-7960cfeb>${ssrInterpolate(formatDate(unref(blogPost).publishedDate))}</p></div></div><hr class="custom-hr" data-v-7960cfeb><h1 data-v-7960cfeb>${ssrInterpolate(unref(blogPost).title)}</h1>`);
              ...
              default: withCtx((_, _push2, _parent2, _scopeId) => {
                if (_push2) {
                  if (!getBlockComponent(block._type)) {
                    _push2(`<p class="text-red-500" data-v-7960cfeb${_scopeId}>Unsupported block type: ${ssrInterpolate(block._type)}</p>`);
                  } else {
                    _push2(`<!---->`);
                  }
                } else {
        ...
        _push(`<div class="text-center" data-v-7960cfeb><p class="text-lg text-gray-500" data-v-7960cfeb>No blog post found.</p></div>`);
      }
      _push(`</div></section><section class="bg-brownBackground py-8 px-8 border-b-2 border-gray-150" data-v-7960cfeb><div class="max-w-screen-lg mx-auto read-more-section px-12 text-center pb-8" data-v-7960cfeb><h2 class="text-2xl font-bold mb-6 text-darkNavy" data-v-7960cfeb>Read More</h2>`);
      if (unref(recentPostsLoading)) {
        _push(`<div class="text-center" data-v-7960cfeb><p class="text-lg text-gray-500" data-v-7960cfeb>Loading recent posts...</p></div>`);
      } else if (unref(recentPostsError)) {
        _push(`<div class="text-center" data-v-7960cfeb><p class="text-lg text-red-500" data-v-7960cfeb>${ssrInterpolate(unref(recentPostsError).message)}</p></div>`);
      } else if (unref(recentPosts) && unref(recentPosts).length) {
        _push(`<div data-v-7960cfeb><div class="grid grid-cols-1 sm:grid-cols-3 gap-6" data-v-7960cfeb><!--[-->`);
        ssrRenderList(unref(recentPosts), (post) => {
          _push(`<div class="post-card hover:shadow-2xl hover:shadow-black border-solid border border-gray-300 transition duration-200 ease-in-out" data-v-7960cfeb>`);
          _push(ssrRenderComponent(_component_NuxtLink, {
            to: `/blog/${post.slug.current}/`
          }, {
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`<img${ssrRenderAttr("src", unref($urlFor)(post.mainImage.asset).width(600).height(400).fit("crop").url())}${ssrRenderAttr("alt", post.mainImage.alt || post.title)} class="w-full h-48 object-cover rounded-t-lg" data-v-7960cfeb${_scopeId}>`);
              } else {
                return [
                  createVNode("img", {
          ...
          _push(`<div class="p-4" data-v-7960cfeb>`);
          _push(ssrRenderComponent(_component_NuxtLink, {
            to: `/blog/${post.slug.current}/`
          }, {
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`<h3 class="text-xl font-semibold text-darkNavy mb-2 hover:text-burntOrange transition-colors duration-200" data-v-7960cfeb${_scopeId}>${ssrInterpolate(post.title)}</h3>`);
              } else {
                return [
          ...
          _push(`<p class="text-sm text-darkGray" data-v-7960cfeb>${ssrInterpolate(formatDate(post.publishedDate))}</p></div></div>`);
        });
        _push(`<!--]--></div></div>`);
      } else {
        _push(`<div class="text-center" data-v-7960cfeb><p class="text-lg text-gray-500" data-v-7960cfeb>No recent posts available.</p></div>`);
      }
...
const _slug_ = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-7960cfeb"]]);

export { _slug_ as default };
//# sourceMappingURL=_slug_-DVgTekNq.mjs.map
